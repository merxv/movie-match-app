<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-800">{{ selectedGenre }} Movies</h1>
      <router-link to="/genres" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 transition">← Back to Genres</router-link>
    </div>
    <div v-if="loading" class="text-center py-8">
      <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
      <p class="mt-2 text-gray-600">Loading {{ selectedGenre }} movies...</p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="movie in filteredMovies" :key="movie._id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow flex flex-col">
        ...
        <div class="p-4 flex flex-col flex-1">
          ...
          <p class="text-sm text-gray-600 mb-2">Genres: {{ movie.genre.join(', ') }}</p>
    ...
    <div v-if="!loading && !error && filteredMovies.length === 0" class="text-center mt-8 text-gray-600">
      No movies in {{ selectedGenre }} yet.
    </div>
  </div>
</template>
...
const selectedGenre = computed(() => route.params.genre || '');
...
const loadMovies = async () => {
  ...
  try {
    ...
    console.log('Genre loaded:', allMovies.value.length, 'Genre:', selectedGenre.value); // Debug
  } catch (err) {
...
const filteredMovies = computed(() => {
  ...
  return allMovies.value.filter(movie => 
    movie.genre && movie.genre.some(g => g.toLowerCase() === selectedGenre.value.toLowerCase())
  );
});
